...
namespace MYFITDAILY_EXE201_Group6.Services.Implementations
{
    public class AiStylistService : IAiStylistService
    {
        ...
        public async Task<ApiResponse<AiRecommendResponseDto>> GenerateOutfitRecommendationAsync(int? userId, AiRecommendRequestDto request)
        {
            ...
            if (!string.IsNullOrWhiteSpace(geminiApiKey))
            {
                ...
                if (geminiResult != null)
                {
                    ...
                    if (isWardrobeEmpty)
                    {
                        geminiResult.EmptyWardrobeMessage = "Hiện tại chưa c� đồ trong tủ của bạn. AI Stylist đã chuẩn bị c�c bộ phối gợi ý mẫu k�m theo để bạn tham khảo hoặc lưu v�o tủ đồ!";
                    }
                    ...
                    bool isFutureEventGemini = request.Timeframe?.Equals("FutureEvent", StringComparison.OrdinalIgnoreCase) == true || !string.IsNullOrWhiteSpace(request.EventNature);
                    if (isFutureEventGemini)
                    {
                        geminiResult.Timeframe = "FutureEvent";
                        geminiResult.EventNature = request.EventNature ?? request.Occasion;
                        geminiResult.JobContext = request.JobContext ?? "Văn ph�ng / C�ng sở";
                        geminiResult.EventWeather = request.EventWeather ?? request.Weather;
                        geminiResult.FutureEventOutfits = GenerateFutureEventOutfitPair(geminiResult.EventNature, geminiResult.JobContext, geminiResult.EventWeather, request.EventDate, pool, isWardrobeEmpty, isMale, user);
                        geminiResult.OutfitName = $"Gợi Ý Sự Kiện Tương Lai: {geminiResult.EventNature}";
                    }
                    ...
                    return ApiResponse<AiRecommendResponseDto>.Ok(geminiResult, isWardrobeEmpty 
                        ? "Hiện tại chưa c� đồ trong tủ của bạn. Đã gợi ý c�c bộ phối mẫu!" 
                        : "AI Stylist đã phối đồ từ ch�nh tủ đồ của bạn k�m c�c bộ phối biến tấu!");
                }
            }
            ...
            bool isFutureEvent = request.Timeframe?.Equals("FutureEvent", StringComparison.OrdinalIgnoreCase) == true || !string.IsNullOrWhiteSpace(request.EventNature);
            FutureEventOutfitPairDto? futureEventOutfits = null;
            if (isFutureEvent)
            {
                var evNature = request.EventNature ?? request.Occasion;
                var evJob = request.JobContext ?? "Văn ph�ng / C�ng sở";
                var evWeather = request.EventWeather ?? request.Weather;
                futureEventOutfits = GenerateFutureEventOutfitPair(evNature, evJob, evWeather, request.EventDate, pool, isWardrobeEmpty, isMale, user);
            }

            var response = new AiRecommendResponseDto
            {
                Id = (int)(DateTimeOffset.UtcNow.ToUnixTimeSeconds() % int.MaxValue),
                OutfitName = isFutureEvent
                    ? $"Gợi Ý Sự Kiện Tương Lai: {request.EventNature ?? request.Occasion}"
                    : (isWardrobeEmpty 
                ...
                HarmonyScore = "98.5%",
                ContrastLevel = "Tỷ Lệ V�ng (Optimal Golden Ratio)",
                CreatedByAi = true,
                Timeframe = isFutureEvent ? "FutureEvent" : "Now",
                EventNature = isFutureEvent ? (request.EventNature ?? request.Occasion) : null,
                JobContext = isFutureEvent ? (request.JobContext ?? "Văn ph�ng / C�ng sở") : null,
                EventWeather = isFutureEvent ? (request.EventWeather ?? request.Weather) : null,
                FutureEventOutfits = futureEventOutfits,
                IsWardrobeEmpty = isWardrobeEmpty,
                EmptyWardrobeMessage = isWardrobeEmpty ? "Hiện tại chưa c� đồ trong tủ của bạn. AI Stylist đã chuẩn bị một số bộ phối gợi ý mẫu k�m theo để bạn tham khảo hoặc th�m v�o tủ đồ!" : null,
        ...
        public async Task<ApiResponse<AiChatResponseDto>> ChatWithStylistAsync(int? userId, AiChatRequestDto request)
        {
            ...
            if (!string.IsNullOrWhiteSpace(geminiApiKey))
            {
                ...
                if (geminiChatResult != null)
                {
                    ...
                    if (isFutureChatEvent)
                    {
                        var evNature = request.EventNature ?? "Sự Kiện Quan Trọng";
                        var evJob = request.JobContext ?? "Văn ph�ng / C�ng sở";
                        var evWeather = request.EventWeather ?? (request.Temperature.HasValue ? $"{request.Temperature}�C, {request.WeatherCondition ?? "Thời tiết sự kiện"}" : "24�C, Tiết trời m�t mẻ");
                        geminiChatResult.Timeframe = "FutureEvent";
            ...
            bool isFutureEventExpert = request.Timeframe?.Equals("FutureEvent", StringComparison.OrdinalIgnoreCase) == true || !string.IsNullOrWhiteSpace(request.EventNature);
            if (isFutureEventExpert)
            {
                var evNature = request.EventNature ?? "Sự Kiện Quan Trọng";
                var evJob = request.JobContext ?? "Văn ph�ng / C�ng sở";
                var evWeather = request.EventWeather ?? (request.Temperature.HasValue ? $"{request.Temperature}�C, {request.WeatherCondition ?? "Thời tiết sự kiện"}" : "24�C, Tiết trời m�t mẻ");
                expertResponse.Timeframe = "FutureEvent";
        ...
        public async Task<ApiResponse<ScanOotdResponseDto>> ScanOotdAsync(ScanOotdRequestDto request)
        {
            ...
            {
                return ApiResponse<ScanOotdResponseDto>.Fail("Vui lòng cung cấp hình ảnh OOTD cần quét");
            }

            // 1. Ưu tiên gọi OpenAI GPT-4o-mini Vision nếu có API Key
            ...
            if (!string.IsNullOrWhiteSpace(openAiApiKey))
            {
                try
                {
                    ...
                    if (openAiResult != null && openAiResult.Items.Count > 0)
                    {
                        ...
                        return ApiResponse<ScanOotdResponseDto>.Ok(openAiResult, "OpenAI GPT-4o Vision đã bóc tách toàn bộ trang phục OOTD thành công!");
                    }
                }
                catch (Exception ex)
                {
            ...
            // 2. Thử gọi Google Gemini 1.5 Flash Vision nếu có API Key
            ...
            if (!string.IsNullOrWhiteSpace(geminiApiKey))
            {
                try
                {
                    ...
                    if (geminiResult != null && geminiResult.Items.Count > 0)
                    {
                        ...
                        return ApiResponse<ScanOotdResponseDto>.Ok(geminiResult, "Gemini Vision đã bóc tách toàn bộ trang phục OOTD thành công!");
                    }
                }
                catch (Exception ex)
                {
            ...
            return ApiResponse<ScanOotdResponseDto>.Ok(fallbackResult, "Fashion Vision Engine đã nhận diện và phân tách các món đồ trong ảnh OOTD thành công!");
        }
        ...
        private static async Task<ScanOotdResponseDto?> CallOpenAiVisionOotdAsync(string apiKey, string imageUrl, string? hint, string? genderHint)
        {
            ...
            var prompt = "Bạn là chuyên gia thị giác máy tính và giám định trang phục thời trang cao cấp (Fashion OOTD Deconstruction AI).\n" +
                         "Hãy quan sát kỹ bức ảnh người dùng chụp toàn thân (OOTD) và bóc tách ĐỒNG LOẠT tất cả các món đồ trang phục mà người đó đang mặc trên người (Áo / Quần hoặc Váy / Áo khoác / Giày dép / Túi xách phụ kiện).\n" +
                         (string.IsNullOrWhiteSpace(hint) ? "" : $"Gợi ý từ người dùng: {hint}\n") +
                         (string.IsNullOrWhiteSpace(genderHint) ? "" : $"Giới tính người mặc: {genderHint}\n") +
                         "Với mỗi món đồ, hãy xác định các trường sau:\n" +
                         "- itemType: Top, Bottom, Outerwear, Shoes, Dress, Accessory\n" +
                         "- brand: Tên thương hiệu nhận diện được (hoặc 'Chưa rõ hãng')\n" +
                         "- name: Tên món đồ cụ thể, thời trang (ví dụ: 'Áo Sơ Mi Poplin Trắng Dài Tay', 'Quần Jeans Ống Suông Vintage Xanh Denim', 'Giày Sneaker Samba Trắng')\n" +
                         ...
                         "- color: Màu sắc chủ đạo (Trắng, Đen, Xanh Denim, Be, Xám...)\n" +
                         "- style: Phong cách (Casual, Minimalist, Streetwear, Formal, Vintage, Sporty, Old Money)\n" +
                         "- season: AllSeason, Summer, Winter, Spring, Fall\n" +
                         "- description: Mô tả ngắn chất liệu và kiểu dáng\n" +
                         "- size: Size ước lượng phù hợp (S, M, L, 29, 30, 40, 41...)\n" +
                         "- suggestedSizes: danh sách size gợi ý\n" +
                         "- confidence: Độ tin cậy (0.85 - 0.99)\n\n" +
                         "Trả về JSON thuần theo cấu trúc:\n" +
                         "{\n" +
                         "  \"overallStyle\": \"Smart Casual\",\n" +
                         "  \"ootdDescription\": \"Bộ trang phục phối thanh lịch và năng động...\",\n" +
        ...
        private static async Task<ScanOotdResponseDto?> CallGeminiVisionOotdAsync(string apiKey, string imageUrl, string? hint, string? genderHint)
        {
            ...
            var prompt = "Bạn là chuyên gia thị giác máy tính và giám định trang phục thời trang cao cấp (Fashion OOTD Deconstruction AI).\n" +
                         ...
                         "- categoryName: Tops, Bottoms, Dresses, Outerwear, Shoes, Accessories\n" +
                         "- color: Màu sắc chủ đạo (Trắng, Đen, Xanh Denim, Be, Xám...)\n" +
                         "- style: Phong cách (Casual, Minimalist, Streetwear, Formal, Vintage, Sporty)\n" +
                         "- season: AllSeason, Summer, Winter, Spring, Fall\n" +
                         "- description: Mô tả ngắn chất liệu và kiểu dáng\n" +
                         "- size: Size ước lượng phù hợp (S, M, L, 29, 30, 40, 41...)\n" +
                         "- suggestedSizes: danh sách size gợi ý\n" +
        ...
        private static ScanOotdResponseDto ClassifyOotdHeuristic(string imageUrl, string? hint, string? genderHint)
        {
            ...
            {
                OverallStyle = isMale ? "Smart Casual Nam Tính" : "Minimalist Chic Thanh Lịch",
                OotdDescription = isMale 
                    ? "Outfit phong cách Smart Casual hiện đại gồm sơ mi trắng phối cùng quần jeans xanh và giày sneaker tối giản."
                    : "Outfit phối sơ mi lụa trắng nhẹ nhàng cùng quần jeans tôn dáng và giày sneaker năng động.",
            ...
            result.Items.Add(new DetectedOotdItemDto
            {
                ...
                CategoryId = 1,
                CategoryName = "Tops",
                Name = isMale ? "Áo Sơ Mi Oxford Trắng Dài Tay" : "Áo Sơ Mi Lụa Trắng Poplin",
                Brand = isMale ? "Ralph Lauren" : "Zara",
                Color = "Trắng",
                Style = "Smart Casual",
                Season = "AllSeason",
                Description = "Chất liệu cotton thoáng mát, đường may đứng phom thanh lịch.",
                Size = isMale ? "L" : "M",
            ...
            result.Items.Add(new DetectedOotdItemDto
            {
                ...
                Name = isMale ? "Quần Jeans Levi's 511 Xanh Indigo" : "Quần Jeans Ống Suông Vintage Xanh Denim",
                Brand = "Levi's",
                Color = "Xanh Denim",
                Style = "Casual",
                Season = "AllSeason",
                Description = "Jeans cạp cao ống suông che khuyết điểm, độ co giãn nhẹ thoải mái.",
                Size = isMale ? "31" : "27",
            ...
            result.Items.Add(new DetectedOotdItemDto
            {
                ...
                CategoryId = 5,
                CategoryName = "Shoes",
                Name = isMale ? "Giày Sneaker Da Trắng Minimalist" : "Giày Sneaker Retro Classic Trắng",
                Brand = "Adidas",
                Color = "Trắng",
                Style = "Casual",
                Season = "AllSeason",
                Description = "Đế cao su êm ái, thiết kế mũi tròn phong cách thể thao cổ điển.",
                Size = isMale ? "41" : "37",
            ...
            // Nếu hint có đề cập đến áo khoác/blazer
            if (lowerHint.Contains("khoác") || lowerHint.Contains("blazer") || lowerHint.Contains("jacket"))
            {
                result.Items.Add(new DetectedOotdItemDto
                {
                    ItemType = "Outerwear",
                    ...
                    Name = isMale ? "Áo Blazer Nam Relaxed Fit Nâu Tây" : "Áo Blazer Nâu Cacao Dạ Mỏng",
                    Brand = "Zara",
                    Color = "Nâu Tây",
                    Style = "Quiet Luxury",
                    Season = "Fall",
                    Description = "Form suông thanh lịch, đệm vai tinh tế tạo tỷ lệ cân đối.",
                    Size = isMale ? "L" : "M",
        ...
        private static void EnsureOotdItemAssets(ScanOotdResponseDto result)
        {
            foreach (var item in result.Items)
            {
                if (string.IsNullOrWhiteSpace(item.ImageUrl))
                {
                    ...
                    {
                        case 1:
                            item.ImageUrl = item.Color.ToLower().Contains("đen") ? "assets/clothes/tshirt_black.svg" : "assets/clothes/shirt_white.svg";
                            break;
                        case 2:
                            item.ImageUrl = item.Color.ToLower().Contains("đen") ? "assets/clothes/pants_black.svg" : "assets/clothes/jeans_blue.svg";
                            break;
                        case 3:
        ...
        public static FutureEventOutfitPairDto GenerateFutureEventOutfitPair(
            string? eventNature,
            string? jobContext,
            string? eventWeather,
            string? eventDate,
            ...
            User? user = null)
        {
            eventNature = !string.IsNullOrWhiteSpace(eventNature) ? eventNature.Trim() : "Tiệc Tối & Sự Kiện Sang Trọng";
            jobContext = !string.IsNullOrWhiteSpace(jobContext) ? jobContext.Trim() : "Văn ph�ng / C�ng sở";
            eventWeather = !string.IsNullOrWhiteSpace(eventWeather) ? eventWeather.Trim() : "24�C, Trời se lạnh m�t mẻ";

            var cleanEvent = RemoveDiacritics(eventNature).ToLowerInvariant();
            var cleanWeather = RemoveDiacritics(eventWeather).ToLowerInvariant();
            var cleanJob = RemoveDiacritics(jobContext).ToLowerInvariant();

            bool isFormal = cleanEvent.Contains("cuoi") || cleanEvent.Contains("tiec") || cleanEvent.Contains("gala") || 
                           cleanEvent.Contains("phong van") || cleanEvent.Contains("hop") || cleanEvent.Contains("hoi thao") ||
                           cleanJob.Contains("lanh dao") || cleanJob.Contains("quan ly") || cleanJob.Contains("cong so");

            bool isOutdoor = cleanEvent.Contains("da ngoai") || cleanEvent.Contains("picnic") || cleanEvent.Contains("the thao") || cleanEvent.Contains("du lich");
            bool isDate = cleanEvent.Contains("hen ho") || cleanEvent.Contains("lang man") || cleanEvent.Contains("date");
            bool isCold = cleanWeather.Contains("lanh") || cleanWeather.Contains("ret") || cleanWeather.Contains("mua") || cleanWeather.Contains("gio");

            // 1. OUTFIT 1: TỪ TỦ ĐỒ CÁ NHÂN (Wardrobe Outfit)
            var defaultWardrobe = isMale ? DEFAULT_WARDROBE_MALE : DEFAULT_WARDROBE_FEMALE;
            List<RecommendedClothingDto> SafeGet(string cat)
            {
                var fromPool = pool.Where(c => c.CategoryName.Equals(cat, StringComparison.OrdinalIgnoreCase)).ToList();
                if (fromPool.Any()) return fromPool;
                return defaultWardrobe.Where(d => d.CategoryName.Equals(cat, StringComparison.OrdinalIgnoreCase)).ToList();
            }

            var safeTops = SafeGet("Tops");
            var safeBottoms = SafeGet("Bottoms");
            var safeShoes = SafeGet("Shoes");
            var safeOuters = SafeGet("Outerwear");

            RecommendedClothingDto topItem;
            RecommendedClothingDto bottomItem;
            RecommendedClothingDto? outerItem = null;
            RecommendedClothingDto shoesItem;

            if (isMale)
            {
                if (isFormal)
                {
                    topItem = safeTops.FirstOrDefault(t => t.Name.Contains("sơ mi", StringComparison.OrdinalIgnoreCase) || t.Style.Contains("Formal", StringComparison.OrdinalIgnoreCase)) ?? safeTops.First();
                    bottomItem = safeBottoms.FirstOrDefault(b => b.Name.Contains("t�y", StringComparison.OrdinalIgnoreCase) || b.Style.Contains("Formal", StringComparison.OrdinalIgnoreCase)) ?? safeBottoms.First();
                    outerItem = safeOuters.FirstOrDefault(o => o.Name.Contains("blazer", StringComparison.OrdinalIgnoreCase)) ?? safeOuters.FirstOrDefault();
                    shoesItem = safeShoes.FirstOrDefault(s => s.Name.Contains("loafer", StringComparison.OrdinalIgnoreCase) || s.Style.Contains("Formal", StringComparison.OrdinalIgnoreCase)) ?? safeShoes.First();
                }
                else if (isDate || !isOutdoor)
                {
                    topItem = safeTops.FirstOrDefault(t => t.Name.Contains("polo", StringComparison.OrdinalIgnoreCase) || t.Name.Contains("len", StringComparison.OrdinalIgnoreCase)) ?? safeTops.First();
                    bottomItem = safeBottoms.FirstOrDefault(b => b.Name.Contains("jeans", StringComparison.OrdinalIgnoreCase) || b.Name.Contains("chino", StringComparison.OrdinalIgnoreCase)) ?? safeBottoms.First();
                    if (isCold) outerItem = safeOuters.FirstOrDefault();
                    shoesItem = safeShoes.FirstOrDefault(s => s.Name.Contains("sneaker", StringComparison.OrdinalIgnoreCase) || s.Name.Contains("loafer", StringComparison.OrdinalIgnoreCase)) ?? safeShoes.First();
                }
                else
                {
                    topItem = safeTops.FirstOrDefault(t => t.Name.Contains("thun", StringComparison.OrdinalIgnoreCase)) ?? safeTops.First();
                    bottomItem = safeBottoms.FirstOrDefault(b => b.Name.Contains("jeans", StringComparison.OrdinalIgnoreCase) || b.Name.Contains("kaki", StringComparison.OrdinalIgnoreCase)) ?? safeBottoms.First();
                    if (isCold) outerItem = safeOuters.FirstOrDefault(o => o.Name.Contains("bomber", StringComparison.OrdinalIgnoreCase) || o.Name.Contains("jacket", StringComparison.OrdinalIgnoreCase));
                    shoesItem = safeShoes.FirstOrDefault(s => s.Name.Contains("sneaker", StringComparison.OrdinalIgnoreCase)) ?? safeShoes.First();
                }
            }
            else
            {
                if (isFormal)
                {
                    topItem = safeTops.FirstOrDefault(t => t.Name.Contains("sơ mi", StringComparison.OrdinalIgnoreCase) || t.Name.Contains("lụa", StringComparison.OrdinalIgnoreCase)) ?? safeTops.First();
                    bottomItem = safeBottoms.FirstOrDefault(b => b.Name.Contains("t�y", StringComparison.OrdinalIgnoreCase) || b.Name.Contains("ch�n v�y", StringComparison.OrdinalIgnoreCase) || b.Name.Contains("midi", StringComparison.OrdinalIgnoreCase)) ?? safeBottoms.First();
                    outerItem = safeOuters.FirstOrDefault(o => o.Name.Contains("blazer", StringComparison.OrdinalIgnoreCase) || o.Name.Contains("tweed", StringComparison.OrdinalIgnoreCase)) ?? safeOuters.FirstOrDefault();
                    shoesItem = safeShoes.FirstOrDefault(s => s.Name.Contains("loafer", StringComparison.OrdinalIgnoreCase) || s.Name.Contains("g�t", StringComparison.OrdinalIgnoreCase)) ?? safeShoes.First();
                }
                else if (isDate)
                {
                    topItem = safeTops.FirstOrDefault(t => t.Name.Contains("lụa", StringComparison.OrdinalIgnoreCase) || t.Name.Contains("len", StringComparison.OrdinalIgnoreCase)) ?? safeTops.First();
                    bottomItem = safeBottoms.FirstOrDefault(b => b.Name.Contains("ch�n v�y", StringComparison.OrdinalIgnoreCase) || b.Name.Contains("jeans", StringComparison.OrdinalIgnoreCase)) ?? safeBottoms.First();
                    if (isCold) outerItem = safeOuters.FirstOrDefault();
                    shoesItem = safeShoes.FirstOrDefault(s => s.Name.Contains("loafer", StringComparison.OrdinalIgnoreCase) || s.Name.Contains("sneaker", StringComparison.OrdinalIgnoreCase)) ?? safeShoes.First();
                }
                else
                {
                    topItem = safeTops.FirstOrDefault(t => t.Name.Contains("thun", StringComparison.OrdinalIgnoreCase) || t.Name.Contains("tee", StringComparison.OrdinalIgnoreCase)) ?? safeTops.First();
                    bottomItem = safeBottoms.FirstOrDefault(b => b.Name.Contains("jeans", StringComparison.OrdinalIgnoreCase)) ?? safeBottoms.First();
                    if (isCold) outerItem = safeOuters.FirstOrDefault();
                    shoesItem = safeShoes.FirstOrDefault(s => s.Name.Contains("sneaker", StringComparison.OrdinalIgnoreCase)) ?? safeShoes.First();
                }
            }

            var wardrobeItems = new List<RecommendedClothingDto> { topItem, bottomItem };
            if (outerItem != null) wardrobeItems.Add(outerItem);
            wardrobeItems.Add(shoesItem);

            var wardrobeOutfit = new AccompanyingOutfitDto
            {
                Id = 501,
                Name = $"Gợi Ý 1 (Tủ Đồ): Set Phối Sẵn Cho {eventNature}",
                Style = isFormal ? "Smart Formal / Business Chic" : (isDate ? "Romantic Semi-Formal" : "Casual Modern"),
                Description = $"Tận dụng đồ c� sẵn trong tủ để tối ưu chi ph�: Phối {topItem.Name} c�ng {bottomItem.Name}{(outerItem != null ? $" v� kho�c ngo�i {outerItem.Name}" : "")}, ho�n thiện với {shoesItem.Name}. Sự kết hợp n�y mang lại phong th�i tự tin, chuy�n nghiệp cho người l�m '{jobContext}' v� th�ch ứng thoải m�i với thời tiết '{eventWeather}'.",
                HarmonyScore = "98%",
                SourceType = "Wardrobe",
                SourceBadge = isMale ? "�� Gợi Ý 1: Từ Tủ Đồ C� Nh�n" : "�� Gợi Ý 1: Từ Tủ Đồ C� Nh�n",
                BodyFlatteringNote = $"✦ Tối ưu v�c d�ng: Tỉ lệ sơ vin t�n ch�n d�i, phom �o đứng dắn tạo chiều s�u thị gi�c, gi�p bạn xuất hiện chỉn chu v� trang nhã nhất tại sự kiện '{eventNature}'.",
                Items = wardrobeItems
            };

            // 2. OUTFIT 2: GỢI Ý TỪ BỘ ĐỒ APP LÀM AFFILIATE (Affiliate Partner Outfit)
            var affiliateOutfit = new AffiliateOutfitDto();
            if (isMale)
            {
                if (isFormal)
                {
                    affiliateOutfit.Id = 601;
                    affiliateOutfit.Name = $"Gợi Ý 2 (Mua Sắm): Bộ Suit & Blazer Sartorial Quý Ông • Coolmate x Routine";
                    affiliateOutfit.Style = "Modern Sartorial • Đối t�c Shopee Mall & Routine";
                    affiliateOutfit.AffiliatePartner = "Routine Official x Coolmate Partner";
                    affiliateOutfit.TotalEstimatedPrice = "1.490.000đ";
                    affiliateOutfit.StylistReason = $"Nếu trang phục trong tủ đồ đã mặc nhiều lần hoặc cần n�ng tầm diện mạo cho sự kiện '{eventNature}', set đồ Affiliate từ đối t�c Coolmate v� Routine mang lại form may đo đứng d�ng, chất liệu tho�ng kh�/giữ ấm th�ch hợp cho '{eventWeather}', khẳng định vị thế '{jobContext}'.";
                    affiliateOutfit.Description = "Set đồ mua sắm li�n kết trọn g�i gồm Sơ mi sợi tre kh�ng khuẩn Coolmate, Quần t�y Smart Pants Routine co giãn 4 chiều, Áo Blazer may đo thời thượng v� Gi�y Penny Loafer da b� cao cấp.";
                    affiliateOutfit.BodyFlatteringNote = "Phom Regular Fit chuẩn tỷ lệ v�ng 4:6, t�n vai �o vu�ng vức v� hack chiều cao hiệu quả.";
                    affiliateOutfit.Items = new List<AffiliateItemDto>
                    {
                        new AffiliateItemDto
                        {
                            Id = 6011,
                            Name = "Áo Sơ Mi Nam Sợi Tre Kh�ng Khuẩn Chống Nhăn Coolmate",
                            CategoryName = "Tops",
                            ImageUrl = "https://images.unsplash.com/photo-1602810318383-e386cc2a3ccf?w=500&auto=format&fit=crop&q=80",
                            Price = "329.000đ",
                            OriginalPrice = "450.000đ",
                            Platform = "Coolmate Official",
                            AffiliateUrl = "https://www.coolmate.me/collection/ao-so-mi-nam",
                            DiscountBadge = "-27%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6012,
                            Name = "Quần T�y Nam Xếp Ly Co Giãn 4 Chiều Routine Smart Pants",
                            CategoryName = "Bottoms",
                            ...
                            Platform = "Routine Mall",
                            AffiliateUrl = "https://routine.vn/quan-nam/quan-tay-nam",
                            DiscountBadge = "-23%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6013,
                            Name = "Áo Kho�c Blazer Nam Relaxed Fit Form Đứng Đẳng Cấp",
                            CategoryName = "Outerwear",
                            ...
                            AffiliateUrl = "https://shopee.vn/search?keyword=blazer%20nam%20may%20do",
                            DiscountBadge = "-28%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6014,
                            Name = "Gi�y Da Loafer Penny Nam Kh�a Ngựa Tinh Tế",
                            CategoryName = "Shoes",
                            ...
                            Platform = "Shopee Mall",
                            AffiliateUrl = "https://shopee.vn/search?keyword=giay%20loafer%20nam%20da%20bo",
                            DiscountBadge = "-29%"
                        }
                    };
                }
                ...
                {
                    affiliateOutfit.Id = 602;
                    affiliateOutfit.Name = $"Gợi Ý 2 (Mua Sắm): Set Clean Fit Trẻ Trung & Thời Thượng • Uniqlo x Shopee";
                    affiliateOutfit.Style = "Korean Clean Fit • Hot Trend Uniqlo & Shopee Mall";
                    affiliateOutfit.AffiliatePartner = "Uniqlo Official x Shopee Partner";
                    affiliateOutfit.TotalEstimatedPrice = "1.190.000đ";
                    affiliateOutfit.StylistReason = $"Set đồ mua sắm đối t�c được phối m�u h�i h�a, chất vải mềm mịn m�t mẻ/giữ ấm th�ch ứng chuẩn x�c với thời tiết '{eventWeather}', đem lại diện mạo năng động, lịch thiệp tại sự kiện '{eventNature}'.";
                    affiliateOutfit.Description = "Gợi ý mua sắm trọn set gồm Áo Polo dệt kim cao cấp, Quần Chinos may đo ống su�ng hack d�ng, Cardigan kho�c nhẹ v� Sneaker retro trắng đế cao su.";
                    affiliateOutfit.BodyFlatteringNote = "Thiết kế năng động, tho�ng m�t, tạo vẻ ngo�i cuốn h�t v� trẻ trung.";
                    affiliateOutfit.Items = new List<AffiliateItemDto>
                    {
                        new AffiliateItemDto
                        {
                            Id = 6021,
                            Name = "Áo Polo Dệt Kim Cổ Bẻ Pique Form Regular Uniqlo",
                            CategoryName = "Tops",
                            ImageUrl = "https://images.unsplash.com/photo-1625910513413-7e54c86d88b0?w=500&auto=format&fit=crop&q=80",
                            Price = "399.000đ",
                            OriginalPrice = "499.000đ",
                            Platform = "Uniqlo VN",
                            AffiliateUrl = "https://www.uniqlo.com/vn/vi/products/polo-shirts",
                            DiscountBadge = "-20%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6022,
                            Name = "Quần Chino Kaki Ống Đứng Regular Fit T�n D�ng",
                            CategoryName = "Bottoms",
                            ...
                            Platform = "Shopee Mall",
                            AffiliateUrl = "https://shopee.vn/search?keyword=quan%20chino%20nam",
                            DiscountBadge = "-24%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6023,
                            Name = "Áo Kho�c Cardigan Len Mỏng Dệt Kim Khuy Gỗ",
                            CategoryName = "Outerwear",
                            ImageUrl = "https://images.unsplash.com/photo-1614975058789-41316d0e2e9c?w=500&auto=format&fit=crop&q=80",
                            Price = "450.000đ",
                            OriginalPrice = "620.000đ",
                            Platform = "Shopee Mall",
                            AffiliateUrl = "https://shopee.vn/search?keyword=cardigan%20nam%20det%20kim",
                            DiscountBadge = "-27%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6024,
                            Name = "Gi�y Sneaker Retro Da Trắng Đế Gum Classic",
                            CategoryName = "Shoes",
                            ImageUrl = "https://images.unsplash.com/photo-1549298916-b41d501d3772?w=500&auto=format&fit=crop&q=80",
                            Price = "490.000đ",
                            OriginalPrice = "690.000đ",
                            Platform = "Shopee Mall",
                            AffiliateUrl = "https://shopee.vn/search?keyword=sneaker%20trang%20retro",
                            DiscountBadge = "-29%"
                        }
                    };
                }
            ...
            else
            {
                ...
                if (isFormal)
                {
                    affiliateOutfit.Id = 603;
                    affiliateOutfit.Name = $"Gợi Ý 2 (Mua Sắm): Set Đầm Lụa & Tweed Quý Ph�i • Ivy Moda x Shopee Mall";
                    affiliateOutfit.Style = "Quiet Luxury / Evening Glamour • Đối t�c Ivy Moda";
                    affiliateOutfit.AffiliatePartner = "Ivy Moda Official x Shopee Mall";
                    affiliateOutfit.TotalEstimatedPrice = "1.580.000đ";
                    affiliateOutfit.StylistReason = $"Set đồ đối t�c được thiết kế độc quyền cho c�c sự kiện trang trọng như '{eventNature}'. Với thời tiết '{eventWeather}', chất lụa cao cấp kết hợp �o tweed kho�c ngo�i giữ ấm nhẹ nh�ng, t�n vinh kh� chất thanh tao của người l�m '{jobContext}'.";
                    affiliateOutfit.Description = "Bộ phối li�n kết mua sắm gồm Đầm lụa satin midi chiết eo Ivy Moda, Áo kho�c Tweed dạ tiểu thư khuy v�ng, Gi�y cao g�t mũi nhọn �m ch�n v� T�i x�ch kẹp n�ch baguette.";
                    affiliateOutfit.BodyFlatteringNote = "Thiết kế chiết eo cao v� xẻ t� tinh tế k�o d�i ch�n, giấu khuyết điểm v�ng 2 tuyệt đối.";
                    affiliateOutfit.Items = new List<AffiliateItemDto>
                    {
                        new AffiliateItemDto
                        {
                            Id = 6031,
                            Name = "Đầm Lụa Satin D�ng Midi Cổ Yếm Chiết Eo Ivy Moda",
                            CategoryName = "Dresses",
                            ...
                            Platform = "Ivy Moda Official",
                            AffiliateUrl = "https://ivymoda.com/danh-muc/dam",
                            DiscountBadge = "-27%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6032,
                            Name = "Áo Kho�c Dạ Tweed Tiểu Thư Khuy Kim Loại Quý Tộc",
                            CategoryName = "Outerwear",
                            ImageUrl = "https://images.unsplash.com/photo-1551028719-00167b16eac5?w=500&auto=format&fit=crop&q=80",
                            Price = "550.000đ",
                            OriginalPrice = "790.000đ",
                            Platform = "Shopee Mall",
                            AffiliateUrl = "https://shopee.vn/search?keyword=ao%20khoac%20tweed%20nu",
                            DiscountBadge = "-30%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6033,
                            Name = "Gi�y Cao G�t Mũi Nhọn 5cm Da B�ng Êm Ch�n Vascara",
                            CategoryName = "Shoes",
                            ImageUrl = "https://images.unsplash.com/photo-1543163521-1bf539c55dd2?w=500&auto=format&fit=crop&q=80",
                            Price = "495.000đ",
                            OriginalPrice = "650.000đ",
                            Platform = "Vascara Official",
                            AffiliateUrl = "https://www.vascara.com/giay-cao-got",
                            DiscountBadge = "-24%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6034,
                            Name = "T�i Da Mini Baguette Đeo Ch�o Thời Thượng",
                            CategoryName = "Accessories",
                            ImageUrl = "https://images.unsplash.com/photo-1584917865442-de89df76afd3?w=500&auto=format&fit=crop&q=80",
                            Price = "299.000đ",
                            OriginalPrice = "420.000đ",
                            Platform = "Shopee Mall",
                            AffiliateUrl = "https://shopee.vn/search?keyword=tui%20baguette%20nu",
                            DiscountBadge = "-29%"
                        }
                    };
                }
                ...
                {
                    affiliateOutfit.Id = 604;
                    affiliateOutfit.Name = $"Gợi Ý 2 (Mua Sắm): Set Parisian Chic & Korean Ulzzang • Uniqlo x Shopee";
                    affiliateOutfit.Style = "Parisian Chic • Đối t�c Uniqlo & Shopee Mall";
                    affiliateOutfit.AffiliatePartner = "Uniqlo VN x Shopee Partner";
                    affiliateOutfit.TotalEstimatedPrice = "1.250.000đ";
                    affiliateOutfit.StylistReason = $"Bộ trang phục mua sắm li�n kết hợp thời, năng động v� duy�n d�ng, ph� hợp tuyệt hảo cho '{eventNature}' trong thời tiết '{eventWeather}', giữ bạn rạng rỡ v� tự tin cả ng�y d�i.";
                    affiliateOutfit.Description = "Set đồ mua sắm li�n kết gồm Sơ mi lụa Poplin d�ng su�ng, Quần Jeans cạp cao wash vintage Uniqlo, Cardigan len dệt kim mềm mịn v� Gi�y Loafer kh�a kim loại.";
                    affiliateOutfit.BodyFlatteringNote = "Áo sơ vin c�ng quần cạp cao gi�p đ�i ch�n tr�ng d�i hơn 5-7cm, tỉ lệ cơ thể cực kỳ c�n đối.";
                    affiliateOutfit.Items = new List<AffiliateItemDto>
                    {
                        new AffiliateItemDto
                        {
                            Id = 6041,
                            Name = "Áo Sơ Mi Lụa Poplin Trắng Form Rộng Tho�ng Kh� Uniqlo",
                            CategoryName = "Tops",
                            ImageUrl = "https://images.unsplash.com/photo-1598033129183-c4f50c736f10?w=500&auto=format&fit=crop&q=80",
                            Price = "499.000đ",
                            OriginalPrice = "650.000đ",
                            Platform = "Uniqlo VN",
                            AffiliateUrl = "https://www.uniqlo.com/vn/vi/women/tops/shirts-and-blouses",
                            DiscountBadge = "-23%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6042,
                            Name = "Quần Jeans Ống Su�ng Vintage Cạp Cao T�n D�ng",
                            CategoryName = "Bottoms",
                            ImageUrl = "https://images.unsplash.com/photo-1541099649105-f69ad21f3246?w=500&auto=format&fit=crop&q=80",
                            Price = "399.000đ",
                            OriginalPrice = "550.000đ",
                            Platform = "Shopee Mall",
                            AffiliateUrl = "https://shopee.vn/search?keyword=quan%20jeans%20suong%20nu%20cap%20cao",
                            DiscountBadge = "-27%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6043,
                            Name = "Áo Kho�c Cardigan Len Sợi Mảnh Dệt Kim Mềm Nhẹ",
                            CategoryName = "Outerwear",
                            ImageUrl = "https://images.unsplash.com/photo-1434389677669-e08b4cac3105?w=500&auto=format&fit=crop&q=80",
                            Price = "350.000đ",
                            OriginalPrice = "480.000đ",
                            Platform = "Shopee Mall",
                            AffiliateUrl = "https://shopee.vn/search?keyword=cardigan%20nu%20det%20kim",
                            DiscountBadge = "-27%"
                        },
                        new AffiliateItemDto
                        {
                            Id = 6044,
                            Name = "Gi�y Loafer Da B�ng Kh�a Ngựa Tối Giản",
                            CategoryName = "Shoes",
                            ImageUrl = "https://images.unsplash.com/photo-1614252235316-8c857d38b5f4?w=500&auto=format&fit=crop&q=80",
                            Price = "450.000đ",
                            OriginalPrice = "620.000đ",
                            Platform = "Shopee Mall",
                            AffiliateUrl = "https://shopee.vn/search?keyword=giay%20loafer%20nu",
                            DiscountBadge = "-27%"
                        }
                    };
                }
            ...
            return new FutureEventOutfitPairDto
            {
                WardrobeOutfit = wardrobeOutfit,
                AffiliateOutfit = affiliateOutfit,
                EventNature = eventNature,
                JobContext = jobContext,
                EventWeather = eventWeather,
                StylistAnalysis = $"✦ AI Stylist đã tổng hợp 2 phương �n tối ưu cho sự kiện '{eventNature}': (1) Tận dụng set đồ trong tủ đồ c� nh�n v� (2) Bộ đồ li�n kết mua sắm độc quyền từ đối t�c Affiliate với gi� ưu đãi. Cả 2 đều đ�p ứng ti�u chuẩn nghề nghiệp '{jobContext}' v� th�ch ứng ho�n hảo với điều kiện thời tiết '{eventWeather}'."
            };
        }
    }
}